			...
			?>
				...
				if(count($res) > 0)
				{
					...
					foreach($res as $row)
					{
						...
						echo '<div><i class="bid-clock"></i> '.date("d-M-Y H:i:s", $row->date_made).'</div>';
				...
				else _e("<p class='box_title_p'>No proposals placed yet.</p>",'ProjectTheme');
				?>	
...
<?php
    ...
    <li class="widget-container widget_text" id="ad-other-details">
       ...
       <?php					
	/*global $current_user;
	$mm = get_currentuserinfo();
	$uid = $current_user->ID;
	$bidding_points = get_user_meta( $uid, 'bidding_points', true ); */
	...
	?>
	<a href="#" class="post_bid_btn_new" id='submit-proposal-id' rel="<?php the_ID(); ?>"><?php _e('Make a Payment','ProjectTheme'); ?></a>
	<?php else:?>	
	<a href="#" class="post_bid_btn_new" id='submit-proposal-id' rel="<?php the_ID(); ?>"><?php _e('Submit a Proposal','ProjectTheme'); ?></a>
	<?php endif; */?>
